...
class Bucketter(object):
    ...
    def topk(self, k, dists):
        topks = []
        for i in tqdm.tqdm(range(len(dists)), f'Top-{k}'):
...
class FPFRandomBucketter(Bucketter):
    ...
    def get_dists(self, embeddings, reps, max_k: int):
    ...
    def bucket(
        ...
        reps = self.get_reps(embeddings, percent_fpf)
        topk_reps, topk_dists = self.get_dists(embeddings, reps, max_k)
